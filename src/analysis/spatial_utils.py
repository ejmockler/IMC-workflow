...
def create_spatial_arrays_from_superpixels(
    ...
    Maps protein expression values from 1D superpixel data to 2D spatial arrays
    using the actual SLIC superpixel shapes. This preserves tissue morphology
    in the visualization instead of showing arbitrary square bins.
    
    Args:
        superpixel_labels: 2D array of superpixel assignments (shape: H x W)
        transformed_arrays: Dict of protein_name -> 1D superpixel values
        superpixel_coords: Coordinates of superpixel centroids (N x 2)
        bounds: Spatial bounds (x_min, x_max, y_min, y_max)
        
    Returns:
        Dictionary of protein_name -> 2D spatial array showing SLIC superpixels
    ...
    if not transformed_arrays:
        raise ValueError("transformed_arrays cannot be empty")
    
    # Get unique superpixel labels (excluding background -1)
    unique_labels = np.unique(superpixel_labels)
    valid_labels = unique_labels[unique_labels >= 0]  # Exclude background (-1)
    
    logger.debug(f"Converting {len(valid_labels)} superpixels to spatial arrays")
    logger.debug(f"Superpixel label range: {unique_labels.min()} to {unique_labels.max()}")
    
    # Check that we have protein data for the superpixels we found
    first_protein = next(iter(transformed_arrays.keys()))
    first_array = transformed_arrays[first_protein]
    
    if first_array.ndim != 1:
        raise ValueError(f"Expected 1D superpixel data, got {first_array.ndim}D")
    
    # Number of superpixels with data vs number of unique labels
    n_data_superpixels = len(first_array)
    n_valid_labels = len(valid_labels)
    
    logger.debug(f"Data superpixels: {n_data_superpixels}, Valid labels: {n_valid_labels}")
    
    # Create spatial arrays for each protein
    spatial_arrays = {}
    
    for protein_name, protein_values in transformed_arrays.items():
        if protein_values.ndim != 1:
            raise ValueError(f"Protein {protein_name} has {protein_values.ndim}D data, expected 1D")
        
        if len(protein_values) != n_data_superpixels:
            raise ValueError(f"Protein {protein_name} has {len(protein_values)} values, expected {n_data_superpixels}")
        
        # Create 2D spatial array initialized with NaN for background
        spatial_array = np.full(superpixel_labels.shape, np.nan, dtype=np.float32)
        
        # Map superpixel values to spatial locations
        # Assumes superpixel indices correspond to the order in valid_labels
        for i, superpixel_value in enumerate(protein_values):
            if i < len(valid_labels):
                superpixel_id = valid_labels[i]
                # Set all pixels belonging to this superpixel to its aggregated value
                mask = superpixel_labels == superpixel_id
                spatial_array[mask] = superpixel_value
            else:
                logger.warning(f"More protein values ({len(protein_values)}) than valid superpixel labels ({len(valid_labels)}) for {protein_name}")
                break
        
        # Count how many pixels were successfully mapped
        n_mapped = np.sum(~np.isnan(spatial_array))
        n_total = spatial_array.size
        n_background = np.sum(superpixel_labels == -1)
        
        logger.debug(f"Protein {protein_name}: {n_mapped}/{n_total - n_background} tissue pixels mapped")
        
        spatial_arrays[protein_name] = spatial_array
    
    logger.info(f"Successfully created spatial arrays for {len(spatial_arrays)} proteins")
    return spatial_arrays