...
**Descriptive focalization discriminator.** The composite categories now carry a **DESCRIPTIVE within-10 µm focalization status** from `composite_focalization.csv`, as defined in `analysis_plans/composite_focalization_spec.md`: `focal` iff D7 self-enrichment is ≥2.0×, otherwise `diffuse`. Among the four structural `interface` categories, all three two-lineage states are focal by D7—endothelial+immune at 2.03×, immune+stromal at 2.58×, and endothelial+stromal at 3.26×—whereas endothelial+immune+stromal is the **lone diffuse interface** at 1.35×. The largest, most-grown interface category therefore does not spatially concentrate, a descriptive pattern consistent with spatial mixing rather than a focal interface; these statuses make no inferential or biological claim.
