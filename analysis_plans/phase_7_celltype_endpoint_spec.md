...
> **Post-remediation supersession note (2026-05-21).** The runtime values cited in the reconciliation block below (1134 rows; 54 demotions; "~66% of superpixels" unassigned at §3.1; `activated_neutrophil_cd44` named in §6.3 rationale) reflect the **pre-remediation Phase 7 cohort** when `activated_neutrophil_cd44` and `activated_neutrophil_cd140b` were active subtypes. Post-remediation (after the spec-literal-gate restoration): the `activated_neutrophil_*` subtypes are removed from the ontology; the neutrophil gate matches the spec literal `+CD45 +Ly6G -CD31 -CD34`; `endpoint_summary.csv` is **840×46** with **263 headlines** and **18 cross-rule demotions across 5 unique endpoint roots**; **~86%** of tissue is unassigned under strict gating; Family A v2 collapses to **4 active typed coordinates + `other_rare` + `unassigned`**. Current canonical numbers: `RESULTS.md`, `METHODS.md`, `endpoint_summary.csv`, `FROZEN_PREREG.md`. **Reconciliation values below are retained as historical pre-registration context, not current runtime.**

> **Pre-run vs. runtime reconciliation (2026-05-07, pre-remediation snapshot).** Several pre-run predictions in this spec did not match the runtime cohort:
