...
**Implementation status (Phase 6 closure, 2026-04-24)**: rule fully implemented in the orchestrator. `run_temporal_interface_analysis.py.run_family_b()` now computes the raw-marker basis alongside the sigmoid basis on every run. Raw-marker rows are stamped with `normalization_mode='sham_reference_raw_marker_per_mouse'` and appear in `endpoint_summary.csv` alongside the sigmoid rows; the Phase 5.2 promise of "both bases emitted in `endpoint_summary.csv`" is now structural, not aspirational. Lineage-source mapping is read from `config.cell_type_annotation.membership_axes.lineages` (markers + aggregation per lineage), so a panel substitution updates the audit by changing config alone. New always-emitted artifacts at `results/biological_analysis/temporal_interfaces/`:
- `family_b_basis_divergence.csv` — one row per (endpoint × composite_label × contrast) with sigmoid g + raw g + `headline_overlap_status` ∈ {`both_above`, `sigmoid_only`, `raw_only`, `both_below`} + `sign_agree`. Headline = clears `|g_shrunk_neutral|>0.5` AND not `g_pathological` AND not `support_sensitive`.
- `family_b_basis_conflict.csv` — opposite-sign-same-endpoint subset (both `headline_overlap_status='both_above'` AND `sign_agree=False`); empty in current pilot.
- `family_b_raw_marker_audit.parquet` — combined sigmoid + raw-marker endpoint rows with `lineage_source` column (mirrors what the standalone `audit_family_b_raw_markers.py` produces; orchestrator emits it on every run).
