...
**Composite focalization discriminator.** Composite categories now carry a **DESCRIPTIVE within-10 µm focalization status** in `results/biological_analysis/composite_focalization.csv`, defined by the pre-registered D7 self-enrichment rule in `analysis_plans/composite_focalization_spec.md` (`focal` at ≥2.0×; otherwise `diffuse`). Among the four structural `interface` categories, the two-lineage interfaces focalize by D7 (endothelial+immune 2.03×, immune+stromal 2.58×, endothelial+stromal 3.26×), whereas triple-positive endothelial+immune+stromal is the lone diffuse interface (1.35×). Thus the largest, most-grown interface category does not spatially concentrate, a descriptive pattern consistent with spatial mixing rather than a focal interface; these statuses make no inferential or biological claim.
