...
19 parquet files (see `docs/DATA_SCHEMA.md`) + `endpoint_summary.csv` (primary reviewer-facing table, 348 rows × 37 columns across Families A/B/C and all 6 pairwise contrasts) + `continuous_sham_pct_sweep.csv` + `family_b_raw_marker_comparison.csv` + `run_provenance.json` (git commit, config hash, file sha256, seeds, parameters, continuous-Sham-reference artifact path + SHA + percentile + aggregation).
