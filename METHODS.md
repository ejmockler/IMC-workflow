...
### Assignment Rate
Mean assignment rate: **~14%** (range 8.9% – 18.8%) across 24 ROIs under the 15-type ontology. The remaining ~86% of superpixels are unassigned — most of which sits at multi-lineage interface tissue that no single-cell-type strict gate accepts. Spatial discrete-cell-type analyses operate on the labelled fraction; the composite-lineage track (8-category interface partition derived from continuous lineage scores; `run_composite_lineage_analysis.py`) covers 100% of tissue including the interface zones.
...
This analysis runs as a **two-tier design**. Tier 1 is the ROI-level significance layer and is the *only* tier that carries permutation p-values; its temporal/regional CSVs are retained purely as a sign+ordering reproduction anchor. Tier 2 is the unweighted **mouse-of-mouse** effect layer (`aggregate_strata`), and it is the basis for every *reported* self-enrichment and region-resolved lineage number. No mouse-of-mouse quantity carries a p-value, FDR, or CI.
...
2. **Marker panel (n=9)**: Limited to coarse lineage identification. ~86% of tissue is unassigned under strict 9-marker gating (lacks marker combination for any single discrete gate). Cannot identify specific T cell subsets, B cells, dendritic cells, or full macrophage polarization spectrum. The panel precludes pathway enrichment analysis (n=8 groundable genes). The composite-lineage track (8-category multi-lineage interface decomposition) recovers 100% of tissue under a coarser-than-celltype labelling.
...
> (a) **Descriptive, mouse-aggregated marker-state composition shifts** across timepoints — notably a higher multi-lineage co-expression fraction at later timepoints (in the composite 8-category interface track this triple-positive growth, ~20% → 38%, is the largest single-category compositional shift, §4b; on the discrete effect-size axis it is one of several comparable retained endpoints at |g_neut| ≈ 1.0, with numerically larger |g| endpoints quarantined as variance-collapse artifacts);
