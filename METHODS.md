...
### Assignment Rate
Mean assignment rate: **~14%** (range 8.9% – 18.8%) across 24 ROIs under the 15-type ontology. The remaining ~86% of superpixels are unassigned; most receive a non-`none` composite lineage tissue-patch state that no single-cell-type strict gate accepts. Spatial discrete-cell-type analyses operate on the labelled fraction; the composite-lineage track (8-category composite partition derived from continuous lineage scores; `run_composite_lineage_analysis.py`) covers 100% of tissue, including the four structural interface categories.

The strict gating convention: **14 of 15 cell-type gates exercise all 9 panel markers** as required positive or required negative; multi-gate ambiguity is 0.0% by construction (no superpixel can satisfy two gates simultaneously). The **neutrophil gate is the named exception**: positive_markers = [CD45, Ly6G], negative_markers = [CD31, CD34] (the Phase 7 spec literal). This leaves CD11b, CD140a, CD140b, CD206, and CD44 free, which is required by the locked Phase 7 v2 pre-registration so that the Family C v2 endpoint `neutrophil_compartment_cd44_rate` (CD44⁺ rate within `cell_type=='neutrophil'` tissue) is non-tautological.

> **Proportions denominator.** Cell type proportions in differential abundance analysis use the total number of superpixels (including unassigned) as denominator. This means changes in the unassigned fraction across timepoints will shift all assigned cell type proportions without a true change in absolute abundance. An alternative denominator (assigned superpixels only) would be equally valid but measures relative composition within the identified fraction rather than tissue-wide prevalence. The total-superpixel denominator is reported; users should interpret proportion changes in the context of the ~86% unassigned tissue, and triangulate against the composite-lineage track for descriptive multi-lineage tissue-patch states.
...
This analysis runs as a **two-tier design**. Tier 1 is the ROI-level significance layer and is the *only* tier that carries permutation p-values; its temporal/regional CSVs are otherwise retained as a sign+ordering reproduction anchor. Tier 2 is the unweighted **mouse-of-mouse** effect layer (`aggregate_strata`), and it is the general basis for *reported* self-enrichment and region-resolved lineage numbers; the separately pre-registered descriptive composite focalization discriminator below is the sole Tier 1 exception. No mouse-of-mouse quantity carries a p-value, FDR, or CI.

**Tier 1 — significance layer (ROI-level).** The only tier that computes permutation p-values.
...
- Aggregation: n_focal-weighted mean enrichment across ROIs, pooled per timepoint (weights = n_focal_cells per ROI). Note: weighted_mean(observed/expected) ≠ weighted_sum(observed)/weighted_sum(expected) by Jensen's inequality. The per-ROI enrichment ratio is the standard estimand (consistent with histoCAT/squidpy); pooled-ratio aggregation would measure a different quantity.
- **Retained as a reproduction anchor, not as the reported basis.** The ROI-weighted pooled-per-timepoint outputs (`results/biological_analysis/spatial_neighborhoods/temporal_neighborhood_enrichments.csv` and `regional_neighborhood_enrichments.csv`, plus the frozen `results/biological_analysis/spatial_neighborhoods_composite/` copy) are RETAINED unchanged as a sign+ordering reproduction anchor for the mouse-of-mouse product (pooled Spearman ρ≈0.97; sign agreement ≈0.985 timepoint / 1.000 region, thresholds documented in `run_stratified_neighborhood_analysis.py`). They exist as a reproducibility check on the ROI-weighted layer, not as the general reported effect basis; the sole exception is the separately pre-registered descriptive composite focalization discriminator below.

**Tier 2 — reported effect layer (mouse-of-mouse).** The basis for the reported self-enrichment table and the region-resolved lineage view. Aggregation is **unweighted** and hierarchical — per-ROI → per-mouse → per-stratum — implemented by `aggregate_strata` in `spatial_neighborhood_analysis.py` and driven by the orchestrator `run_stratified_neighborhood_analysis.py`. A stratum × focal × neighbor cell is emitted when at least **2 effective mice** contribute a value: `n_mice_effective` counts mice that actually produced a value, not mice merely row-present. Support is handled by **emit-not-drop** — sparse strata are FLAGGED (`below_min_support` when n_focal_cells falls below the support floor; `insufficient_support` when fewer than 2 effective mice) and carried in the output, never removed. This tier reports **descriptive effect size only** (enrichment ratio / log2 enrichment) with explicitly **no significance, p-value, or CI at thin strata** — the descriptive n=2 mice/timepoint framing holds throughout.
- Products (all under `results/biological_analysis/spatial_neighborhoods/`): `stratified_neighborhood_enrichments.csv` is the canonical superset with `strata_level ∈ {pooled, timepoint, region, region_x_timepoint}`; `temporal_neighborhood_enrichments_mouse.csv` (the `timepoint` slice) and `regional_neighborhood_enrichments_mouse.csv` (the `region` slice) are derived from it; `lineage_neighborhood_by_region_timepoint.csv` is the region × timepoint lineage lens. This is the design for the reported self-enrichment and region-resolved lineage surfaces; repo-wide reconciliation of every dependent table is complete — the §4 self-enrichment table, the INDRA concordance/§6, and the region-lens are mouse-of-mouse, while the §4 cross-type D1 tables and the §4b composite views in RESULTS.md are retained as explicitly labeled ROI-level reproduction anchors.

**Composite focalization discriminator.** Composite categories now carry a **DESCRIPTIVE within-10 µm focalization status** in `results/biological_analysis/composite_focalization.csv`, defined by the pre-registered D7 self-enrichment rule in `analysis_plans/composite_focalization_spec.md` (`focal` at ≥2.0×; otherwise `diffuse`). Among the four structural `interface` categories, endothelial+stromal (3.26×) and immune+stromal (2.58×) focalize by D7 and triple-positive endothelial+immune+stromal is diffuse (1.35×). The endothelial+immune label is **not robust to the kNN neighborhood size k** and the earlier "lone diffuse interface" framing is withdrawn: a pre-registered sweep (`analysis_plans/k_sensitivity_precommitment.md`; `results/biological_analysis/k_sensitivity_focalization.csv`) gives 2.24× (focal) at k=5, 1.99× (diffuse) at k=10, 1.74× (diffuse) at k=20, and its status also differs by aggregation basis at fixed k=10. Stable across k ∈ {5,10,20} are the rank order (endothelial+stromal > immune+stromal > endothelial+immune > triple-positive), the focal status of endothelial+stromal and immune+stromal, and the diffuse status of triple-positive. k=10 remains the primary value; the sweep is a disclosure, not a re-selection. Thus the largest, most-grown interface category does not spatially concentrate, a descriptive pattern consistent with spatial mixing rather than a focal interface; these statuses make no inferential or biological claim.

At DNA-superpixel resolution, co-positivity is a co-localization tissue-patch state, not a cell phenotype or an inferential biological claim. Interpretation is limited by n=2 mice per timepoint and confounded by DNA-only SLIC segmentation, no spillover compensation, and injury-driven cellularity. The **dilution corollary** is that cross-scale robustness is not evidence of interface biology: genuine focal co-expression dilutes at coarser grain, so scale-robust co-positivity indicates diffuse mixing, not a validated interface. CD44 and CD140b are excluded from these composite lineage-category definitions, so the neutrophil–CD44 headline remains non-tautological.

> **Marker sharing and self-enrichment.** Several cell type definitions in the current 15-type ontology share positive markers (e.g., `activated_endothelial_cd44` and `activated_fibroblast_cd44` both require CD44+; `m2_macrophage` and `activated_m2_cd44`/`activated_m2_cd140b` share CD11b+ and CD206+). Because boolean gating assigns positivity based on continuous expression thresholds, cell types sharing markers will co-localize in marker expression space and consequently in physical space, producing self-enrichment that reflects shared gating criteria rather than independent biological co-localization. Self-enrichment scores (diagonal of the enrichment matrix) should be interpreted with this confound in mind.
...
The three continuous "lineage" scores are **marker-state axes** — single/double-marker state indices, not cell-lineage identifications (immune = CD45 alone [max]; endothelial = mean(CD31, CD34); stromal = CD140a alone [max]). They become continuous memberships through analyst-set knobs — `sigmoid_steepness` = 10.0, center = Sham per-mouse 60th percentile, scale = experiment-wide IQR — which are fixed by the analyst, frozen for follow-up cohorts, and directly shape the reported magnitudes. This continuous-membership surface is the **least-bad, fully-disclosed, parameter-tuned** view (one of several): it recovers the multi-lineage co-expression that strict discrete gating discards, but it still inherits ~10 μm superpixel mixing, the tuned sigmoid parameters above, and n=2 fragility, and is reported on those terms — not as a robust, primary, or independently defensible quantitative currency for the analysis as a whole. (This whole-analysis framing is distinct from, and does not disturb, the within-Family-A and within-Family-B sigmoid-vs-raw normalization-path labels below.)

**Marker-role assignment — convention, and its sensitivity.** Each panel marker is used in exactly one role: lineage (CD45; CD31+CD34; CD140a), within-immune subtype (CD11b, Ly6G, CD206), or activation overlay (CD44, CD140b). No recorded rationale exists for the single-marker composition, for CD140b (PDGFRβ, a mural/pericyte marker) sitting in activation rather than stromal, or for `mean` on endothelial where the others use `max`; the implementation accepts arbitrary marker lists and max/mean/min, so all three are conventions rather than constraints. `run_marker_assignment_sensitivity.py` recomputes all three axes under four assignments (current; multi-marker positives; CD140b added to stromal; endothelial by max) against the same frozen sigmoid parameters and persisted arrays → `results/biological_analysis/marker_assignment_sensitivity.csv`. **The Sham→D7 direction is identical for all three axes under every variant**; absolute levels shift (a max over more markers raises every score including the Sham baseline), so levels are comparable only within a variant. Direction of the current convention's effect: it *increases* the apparent immune rise (+0.219 vs +0.141 with CD11b added) and endothelial rise (+0.181 vs +0.140 under max), and *decreases* the stromal rise (+0.145 vs +0.170 with CD140b added). The stromal axis is therefore conservative as configured; the endothelial axis is the one flattered by both this convention and the cofactor drift noted above, and is reported on those terms.
...
- **Family A — Interface composition (compositional).** *v1 (`endpoint_axis='composite_label_8cat'`):* Mouse-level fraction of superpixels in each of 8 composite categories {`immune`, `endothelial`, `stromal`, `endothelial+immune`, `immune+stromal`, `endothelial+stromal`, `endothelial+immune+stromal`, `none`} (these are the exact column names in `interface_fractions.parquet`; "triple" is human shorthand for the 3-lineage `endothelial+immune+stromal` category). CLR transform with **Bayesian-multiplicative zero replacement** (Martín-Fernández et al. 2003, Dirichlet α=0.5) rather than additive ε. Minimum-prevalence filter (<1% in all timepoints → collapsed to "other_rare"). Two normalization paths reported side-by-side (Phase 1 replaced the earlier per-ROI sigmoid with a Sham-reference-centered sigmoid; both normalization paths are now Sham-anchored): (i) **Sham-reference sigmoid on continuous lineage memberships** (primary, normalization_mode=`per_roi_sigmoid` for legacy compatibility — the underlying sigmoid is Sham-anchored since Phase 1); (ii) **raw-marker Sham-reference percentile** (corroboration, normalization_mode=`sham_reference_raw_marker_per_mouse`). Sensitivity sweeps: lineage threshold {0.2, 0.3, 0.4}; raw-marker Sham percentile {65, 75, 85}; continuous Sham percentile {50, 60, 70} (Phase 1.5b — closed). Disagreement reported per row via `normalization_sign_reverse` AND symmetric `normalization_magnitude_disagree` (≥2× divergence). `clr_none_sensitivity` flags rows whose sign flips when the `none` category is excluded (Phase 1.5a — closed; 0/48 flips). *v2 (`endpoint_axis='discrete_celltype_16cat'`, Phase 7):* parallel CLR over up to 16 discrete cell-type coordinates (15 typed + `unassigned`), with min-prevalence collapse to `other_rare` (sweep at {0.005, 0.01, 0.02}); 4 active typed coordinates + `other_rare` + `unassigned` in the current post-remediation pilot (down from 13 pre-remediation, because the spec-literal-gate restoration increased neutrophil prevalence and pushed the activated_*_cd44 / cd140b subtypes below the 1% threshold). Single-path (no v2-internal corroborator); rule = `|g|>0.5 AND not g_pathological`, tagged `headline_rule_version='v2_pathology_only'`. The v1/v2 cross-rule (`headline_demoted_reason='cross_axis_co_headline_forbidden'`) prevents v2 from co-headlining v1 on the same biological event; 18/840 rows demoted in the current cohort (5 unique endpoint roots: 3 Family A + 15 Family B row-level demotions).
- **Family B — Continuous neighborhood lineage shifts.** *v1 (`stratifier_basis='composite_label'`):* Per (composite_label × neighbor_lineage), mouse-level mean of `neighbor_lineage_X − self_lineage_X` (neighbor-minus-self delta). Minimum support filter `n_superpixels ≥ 20` per row; filtered rows preserved as NaN-with-flag distinguishing `absent_biology` from `below_min_support`. Only temporal changes in the delta are interpretable (self-delta is biased by definition). Sensitivity sweep: `{10, 20, 40}` minimum support; `support_sensitive=True` flag on rows whose presence changes across the sweep. Phase 1.5c added a sigmoid-independent **raw-marker basis** (`audit_family_b_raw_markers.py`) using config-defined raw-arcsinh composites for each lineage; Phase 5.2 amends the rule for follow-up cohorts to **co-primary intersection-conservative** reporting (both bases emit endpoints; the intersection is the conservative headline set; sigmoid-only and raw-only sets are reported but flagged as basis-dependent). Pilot v1 result: 21 sigmoid + 18 raw-marker Sham→D7 endpoints clear `|g_shrunk_neutral|>0.5`; 14 in common; 96% sign agreement on overlapping endpoints. *v2 (`stratifier_basis='discrete_celltype'`, Phase 7):* same kNN gradient operator with `cell_type` as the stratifier instead of `composite_label`; both sigmoid and raw-marker bases reported under `normalization_mode`. `support_sensitive=True` flagged on 126/720 Family B rows in the current cohort (90 v1 + 36 v2).
- **Family C — Cross-compartment activation.** *v1:* Mouse-level CD44⁺ rate within {CD45⁺, CD31⁺, CD140b⁺, background} compartments and triple-overlap fraction, where compartment positivity uses **Sham-reference** 75ᵗʰ-percentile thresholds computed once on Sham-only ROIs (prevents outcome contamination from D7-elevated markers driving the threshold). Sensitivity sweep: `{65, 75, 85}` Sham percentile. *v2 (Phase 7):* single-row neutrophil compartment extension (`neutrophil_compartment_cd44_rate`) using `cell_type=='neutrophil'` from the discrete annotation. The 14 other discrete cell types pin CD44 status by gate construction (rate would be 0 or 1 by definition); only `neutrophil` admits a non-tautological measurement.
...
**22 parquet files** (see `docs/DATA_SCHEMA.md`) + `endpoint_summary.csv` (primary reviewer-facing table, **840 rows × 46 columns** post-Phase-7 across Families A/B/C v1+v2 and all 6 pairwise contrasts; Family A: 48 v1 + 36 v2 = 84, Family B: 540 v1 + 180 v2 = 720, Family C: 36) + `continuous_sham_pct_sweep.csv` + `family_b_raw_marker_comparison.csv` + `run_provenance.json` (git commit, config hash, file sha256, seeds, parameters, continuous-Sham-reference artifact path + SHA + percentile + aggregation).
...
### Minimal defensible claim set

> **What this 9-marker / n=2 / superpixel design CAN support (descriptive, hypothesis-generating only):**
> (a) **Descriptive, mouse-aggregated marker-state composition shifts** across timepoints — notably a higher multi-lineage co-expression fraction at later timepoints (in the composite 8-category track this triple-positive growth, ~20% → 38%, is the largest single-category compositional shift, §4b; on the discrete effect-size axis it is one of several comparable retained endpoints at |g_neut| ≈ 1.0, with numerically larger |g| endpoints quarantined as variance-collapse artifacts);
> (b) **CD44 read as a pan-compartment activation gradient** (literature-consistent), reported as a gradient rather than a cell-type-specific marker;
> (c) **Real spatial structure in the marker fields** — continuous-field Moran's I and join-count statistics — reported as **spatial coherence, not "cell–cell interaction"**;
> (d) **Candidate effect magnitudes and n_required for a powered follow-up cohort.** The 263 `|g| > 0.5` headline rows are **candidates** against the full 840-endpoint denominator, not confirmed effects; the skeptical prior (under which nothing exceeds |g| ≈ 0.32) is reported beside the neutral planning default, so the honest lower bound travels with every headline number;
> (e) **Panel coverage itself as a result** — under this panel ~86% of tissue is marker-unresolved.
>
> Within that claim set two narrowings apply, replacing any blanket ban on discrete-across-time comparison: discrete cell-type proportions are **descriptive-of-segmentation, never significance-bearing**, and `c:mixed` is a **reproducible multi-lineage co-expression class, never a cell type**.
>
> **What this design CANNOT support:** cell identities; cell–cell interactions; statistical significance; mechanism; fibroblast-vs-pericyte discrimination; epithelial-identity claims; cross-scale persistence; or "validation". In particular the panel carries no tubular / epithelial marker, so it excludes tubular epithelium as a testable referent — the ~86% marker-unresolved fraction cannot be assigned any single biological referent, and the external UUO-literature expectation that this fraction is an epithelial compartment remains a hypothesis this panel cannot test.
