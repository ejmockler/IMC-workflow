...
| `normalization_mode` | str | Family A v1: `per_roi_sigmoid` only (legacy label retained on the primary v1 sigmoid path; the underlying sigmoid is Sham-anchored since Phase 1; the raw-marker corroboration is column-encoded as `hedges_g_sham_ref` on the same row, not as a separate row). Family A v2: NaN (single-path, discrete cell-type CLR). Family B: `sham_reference_v2_continuous` \| `sham_reference_raw_marker_per_mouse` (dual-basis Phase 6). Family C: `sham_reference_raw_marker_per_mouse`. |
| `sham_percentile` | float | Sham percentile actually carried on the row: **60** (Family A v1 primary; matches the Sham-reference artifact `_metadata.percentile=60.0`); NaN (Family A v2); **75** (Family C primary); NaN (Family B v1 + v2 — Family B does not carry a per-row Sham percentile). The 65/75/85 sensitivity sweeps for Families A and C live in `family_a_endpoints_norm_sweep.parquet` and `family_c_sensitivity_endpoints.parquet`, not in this column. |
