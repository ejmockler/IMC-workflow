...
**Family B (primary, sigmoid Sham-ref continuous lineage scores) produced no endpoint clearing |g_shrunk_neutral| > 0.5 at Sham→D7.** Pre-registered and run; not omitted. The absence of Family B primary-path headlines is itself a result.

**Family B sigmoid-independent sensitivity check (Phase 1.5c — post-hoc discovery, not pre-registered).** A parallel Family B computed on raw arcsinh markers instead of sigmoid lineage scores (`audit_family_b_raw_markers.py`) surfaces 18 Sham→D7 endpoints at |g_neut| > 0.5 (none `g_pathological`), concentrated on `vs_sham_mean_delta_lineage_immune` — neighbors of various composite-label superpixels acquire more immune-marker expression at D7 than the center. This is consistent with published UUO biology (immune infiltration into non-immune compartments) but was **not pre-registered as a primary path**; it is reported as a sensitivity-analysis discovery. The sigmoid/raw disagreement (55/166 endpoints ≥2× magnitude divergence; 8/166 sign reversals at |g|>0.5) is consistent with sigmoid saturation: most superpixels bimodalize on lineage scores, compressing neighbor-minus-self to near-zero outside interface regions. Raw markers vary continuously and retain dynamic range. Follow-up cohorts should pre-register whichever lineage-source basis (sigmoid or raw) they intend to report as primary for Family B.
