...
The v2 set extends — and in some places contradicts — the v1 candidate read. The bare `endothelial_clr` (v2 discrete-celltype) rises Sham→D7 (+0.77 g_neut) while v1's `endothelial_clr` (single-lineage interface category in the 8-cat simplex) is filtered out by `normalization_sign_reverse` (both near zero). The v2 result reflects the discrete bare-`endothelial` cell-type proportion rising 3.82% → 7.86% (mouse-mean across 6 ROIs/timepoint), which the 8-cat lineage-interface CLR doesn't surface because it pools the bare lineage with all its overlaps. Phase 7 v2 operates on a closed simplex of 13 discrete cell-type coordinates (after min-prevalence collapse to `other_rare`), with `unassigned` carrying ~65.7% of the simplex mass on the mouse-level mean (Sham 68.5% → D7 62.7%, a 5.8-percentage-point drop).
...
**Rank-based selection-free companion.** `temporal_top_ranked_by_effect.csv` ranks on |g_shrunk_neutral|, with `g_pathological` rows (|g|>3 AND pooled_std<0.01) quarantined. Sign convention: `g = (mean_Sham − mean_D7) / pooled_std`, so a negative g means D7 > Sham. Sham→D7 top-5: `activated_myeloid_cd44` (+0.56, D7 < Sham), `neutrophil` (−0.55, **D7 > Sham** — proportion rises 4.82% → 6.85%), `fibroblast` (+0.54, D7 < Sham), `endothelial` (−0.53, D7 > Sham — proportion rises 3.82% → 7.86%), `activated_fibroblast_cd140b` (+0.44, D7 < Sham). `immune_cells` is ranked-but-quarantined as `g_pathological=True` (|g|=3.98 with pooled_std=0.00128; one Sham mouse has near-zero variance).
