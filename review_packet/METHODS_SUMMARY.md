...
- `endpoint_summary.csv` (**840 rows × 46 cols** post-remediation): every endpoint with shrunken g columns, pathology flag, insufficient-support flag, threshold-sensitive flag, normalization flags (Family A), `sham_percentile` (Family A per-roi 60; Family C 65/75/85), Phase 7 v2 schema columns (`endpoint_axis`, `stratifier_basis`, `headline_rule_version`, `headline_demoted_reason`, `is_headline`, `unassigned_rate_mouse_mean_1/2`, `min_prevalence_sweep_value`).
